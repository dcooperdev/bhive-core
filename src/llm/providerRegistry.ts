...
export interface ProviderDescriptor {
  ...
  create: (apiKey: string | undefined, model: string) => LLMAdapter;
}
...
export const PROVIDER_REGISTRY: Record<LLMProviderName, ProviderDescriptor> = {
  ...
  openai: {
    ...
    toolCalling: 'full',
    create: (apiKey, model) => new OpenAIAdapter(apiKey, model)
  },
  anthropic: {
    name: 'anthropic',
    ...
    toolCalling: 'full',
    create: (apiKey, model) => new AnthropicAdapter(apiKey, model)
  },
  ollama: {
    name: 'ollama',
    ...
    envVar: null,
    toolCalling: 'basic',
    create: (_apiKey, model) => new OllamaAdapter(model)
  }
};
...
/** Builds the adapter for `provider`, defaulting the model from the registry when `model` is omitted. */
export function createLLMAdapter(provider: LLMProviderName, apiKey?: string, model?: string): LLMAdapter {
  ...
  return descriptor.create(apiKey, model ?? descriptor.defaultModel);
}
